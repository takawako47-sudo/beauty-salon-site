...
import { getCalendarEvents, getAvailabilitySlots, AvailabilitySlot, CalendarEvent } from "@/lib/calendar";
...
export default function ReservePage() {
  ...
  // コンポーネントマウント時にイベントを取得 (Client-side fetching for interactivity)
  useEffect(() => {
  ...
  // 選択された日付のスロットを生成
  const slots = getAvailabilitySlots(events, selectedDate);
    ...
    <>
      ...
      <main style={{ padding: "40px 20px", textAlign: "center", minHeight: "80vh", backgroundColor: "#f9f9f9" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", backgroundColor: "#fff", padding: "30px", borderRadius: "10px", boxShadow: "0 4px 6px rgba(0,0,0,0.05)" }}>
          ...
          {loading ? (
            <p style={{ padding: "40px", color: "#999" }}>情報を取得中...</p>
          ) : (
